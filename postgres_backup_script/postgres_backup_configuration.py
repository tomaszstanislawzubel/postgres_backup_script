...
def smb_share_validation():
    ...
    with open('/etc/fstab') as f:
        if '//1.2.3.4/smbshare /media/backup cifs credentials=/path/to/.smbcredentials,iocharset=utf8 0 0' in f.read():
            pass
        else:
            with open('/etc/fstab', 'a') as f:
                f.write('//1.2.3.4/smbshare /media/backup cifs credentials=/path/to/.smbcredentials,iocharset=utf8 0 0')
...
def backup_task(occurence, occurence_short, occurence_seconds):
    ...
    for file in os.listdir("."):
        if file.endswith(".sql"):
            compression_level = 9
            pyminizip.compress(file, ".", str(file).strip('.sql')+"{}.zip".format(occurence_short), "password", compression_level)
